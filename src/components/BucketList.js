...
class BucketList extends Component {
    ...
    displayBucketList(){
      ...
      console.log("Called")
      axios.get(`${this.BASE_URL}bucketlists/?page=${this.state.page}&limit=${this.state.limit}`,  { headers: { Authorization: `${window.localStorage.getItem('token')}` } })
            .then(data =>{
                console.log(data)
                if(data.data.success){
                  console.log(data.data)
                  this.setState({...this.state, bucketlist : data.data.bucketlist, loading: false})
    ...
    changePage(value){
      this.setState({...this.state, page: value.target.value})
      this.displayBucketList()
    ...
    changeLimit(e){
      ...
      console.log(this.state.limit)
        setTimeout(this.displayBucketList.bind(this), 200)
  ...
  render() {
    ...
    return ( 
      ...
      <Navigation />
        ...
        <div className="limit"><span>Number of Bucketlist to show: <input type="number" value={this.state.limit} onChange={this.changeLimit.bind(this)} onKeyUp={value => this.changeLimit(value)}></input></span></div>
        <div className="limit"><span>Search BucketLists: <input type="text" onChange={this.searchList.bind(this)} onKeyUp={value => this.searchList(value)}></input></span></div>
        <div className="buckets">
        
        <div className="bucket-list">
        {
          this.state.loading === false? this.renderList() : this.renderLoading()
        }
        </div>
        </div>
        </header>
