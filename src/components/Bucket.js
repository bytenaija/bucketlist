...
class Login extends Component {
    ...
    componentDidMount(){
        ...
        if(this.state.loggedIn){
            axios.get(`${this.BASE_URL}bucketlists/${this.props.match.params.id}`,  { headers: { Authorization: `${window.localStorage.getItem('token')}` } })
            .then(data =>{
                console.log(data)
    ...
    renderAuthor(){
        return <span>{this.state.bucketlist.created_by.map((author) => author.email)} </span>
    }

    renderItems(){
        return <div>{this.state.bucketlist.items.map((item) => <li><Link to={{ pathname: '/bucketlists/' + this.props.match.params.id + '/' + item._id }}> {item.name}</Link></li>)}</div>
    }
  render() {
    ...
    return ( 
      ...
      <Navigation />
        ...
        <div className="one-buckets">
        <div className="card">
          <div className="card-header">
          <img src={this.img} alt={this.state.bucketlist.name}/>
          <h2>{this.state.bucketlist.name}</h2>
          <div className="card-body">
          <h3>BucketList Items</h3>
            {this.state.bucketlist.items? this.renderItems(): ''}
          </div>
          <div className="card-footer">
            <span>Created by: {
                this.state.bucketlist.created_by ? this.renderAuthor() : 'Anonymous'
            }
            </span>
          </div>
          </div>
